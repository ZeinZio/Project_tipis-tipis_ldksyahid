...
class TrackVisitor
{
    ...
    private function track(Request $request): void
    {
        ...
        if (\Illuminate\Support\Facades\DB::connection()->getDriverName() === 'sqlite') {
            $stat = DB::table('tr_visitor_page_stat')->where('statDate', $date)->where('path', $path)->first();
            if ($stat) {
                DB::table('tr_visitor_page_stat')->where('statDate', $date)->where('path', $path)->update([
                    'totalHits' => DB::raw('totalHits + 1'),
                    'uniqueVisitors' => DB::raw('uniqueVisitors + ' . $isUniqueOnPage),
