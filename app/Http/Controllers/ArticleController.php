...
class ArticleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('created_year')) {
            ...
            $yearRaw = $isSqlite ? "strftime('%Y', created_at)" : "YEAR(created_at)";
            $query->whereRaw($yearRaw . ' IN (' . implode(',', array_map('intval', $years)) . ')');
        }
        ...
        $isSqlite = \Illuminate\Support\Facades\DB::connection()->getDriverName() === 'sqlite';
        $yearFunc = $isSqlite ? "strftime('%Y', created_at) as year" : "YEAR(created_at) as year";
        $years   = Article::selectRaw($yearFunc)->distinct()->orderByDesc('year')->pluck('year');
